...
export function PropertiesTable({ data: initialData, is_edit_mode }: { data: IBaseValue[] | IProperty[], is_edit_mode: boolean}) {
  ...
  const columns: ColumnDef<IBaseValue | IProperty>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <div className="flex content-center space-x-4 w-full">
          {[1].map((points) => (<Button key={points} className="w-1/4 flex-1 bg-black hover:bg-gray-300 hover:text-black text-white rounded-lg" onClick={() => try_increase_skill(row.original, points)}>{points}</Button>))}
      </div>
    },
  ]
