...
export class CreateCharacterUseCase implements UseCase<CreateCharacterInput, CreateCharacterOutput> {
  ...
  async execute(input: CreateCharacterInput): Promise<Result<CreateCharacterOutput, Error>> {
    try {
      ...
      const createResult = await this.characterService.createCharacter(input.characterData, input.idToken);
   ...
   * Validates character creation data according to game rules
   */
  private validateCharacterCreationData(characterData: PostCharactersRequest): { isValid: boolean; error?: string } {
    // Basic validation - in a real implementation, this would be more comprehensive
    const name = characterData.generalInformation.name;
    ...
    if (characterData.attributes) {
      for (const [attrName, attrValue] of Object.entries(characterData.attributes)) {
        if (typeof attrValue === "object" && attrValue !== null && "current" in attrValue) {
          const value = (attrValue as { current: unknown }).current;
