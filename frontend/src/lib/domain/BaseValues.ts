...
export class BaseValueCollection {
  ...
  getAllBaseValues(): BaseValueViewModel[] {
    return Object.entries(this.baseValues).map(([name, baseValue]) => 
      this.createBaseValueViewModel(name, baseValue)
    );
  }
  ...
  getEssentialBaseValues(): BaseValueViewModel[] {
    ...
    return essentialNames
      .map(name => this.getBaseValue(name))
      .filter((bv): bv is BaseValueViewModel => bv !== null);
  }
