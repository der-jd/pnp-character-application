resource "aws_cloudwatch_log_group" "update_base_value_state_machine_log_group" {
  name              = "/aws/vendedlogs/states/update-base-value"
  retention_in_days = 0
}

resource "aws_sfn_state_machine" "update_base_value_state_machine" {
  name     = "update-base-value"
  role_arn = aws_iam_role.step_function_role.arn
  type     = "EXPRESS"
  logging_configuration {
    log_destination        = "${aws_cloudwatch_log_group.update_base_value_state_machine_log_group.arn}:*"
  ...
  definition = jsonencode({
    StartAt = "UpdateBaseValue",
    States = {
      UpdateBaseValue = {
        Type          = "Task",
        QueryLanguage = "JSONata",
        Resource      = module.update_base_value_lambda.lambda_function.arn,
        Assign = {
          statusCode          = "{% $states.result.statusCode %}",
          updateBaseValueBody = "{% $states.result.body %}"
        },
      ...
      SuccessState = {
        ...
        Output = {
           ...
           */
          "body" = "{% $string({'data': $parse($updateBaseValueBody),'historyRecord': $addHistoryRecordBody ? $parse($addHistoryRecordBody) : null}) %}"
        }
      }
    }
  })
