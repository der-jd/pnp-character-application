resource "aws_lambda_function" "increase_skill_lambda" {
  ...
  filename = "../backend/dist/increase-skill-${var.git_commit_hash}.zip" // TODO code changes are not automatically applied with terraform apply --> use zip file name based on git hash?!
}
